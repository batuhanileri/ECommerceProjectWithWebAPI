...
namespace WebAPIWithCoreMvc.ApiService
{
    public class CategoryApiService
    {
        ...
        public async Task<IEnumerable<CategoryWithProductDto>> GetAllAsyncs(int id)
        {
            IEnumerable<CategoryWithProductDto> categoryDtos;

            var response = await _httpClient.GetAsync($"categories/{id}");

            if (response.IsSuccessStatusCode)
            {
                categoryDtos = JsonConvert.DeserializeObject<IEnumerable<CategoryWithProductDto>>(await response.Content.ReadAsStringAsync());
            }
            else
            {
                categoryDtos = null;
